...
public class Asiakkaat extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(pathInfo==null) { 											//Haetaan kaikki asiakkaat 
			...
			String asiakas_id = pathInfo.replace("/haeyksi/", "");		 //poistetaan polusta "/haeyksi/", j‰ljelle j‰‰ asikas_id		
			Asiakas asiakas = dao.etsiAsiakas(asiakas_id);
			if (asiakas==null) {
				strJSON = "{}";
			} else {
				JSONObject JSON = new JSONObject();
				JSON.put("asikas_id", asiakas.getAsiakas_id());
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out = response.getWriter();
		Dao dao = new Dao();
		if(dao.lisaaAsiakas(asiakas)) {	// metodi paluattaa true/false
			out.println("{\"response\":1}");	//Asiakkan lis‰‰minen onnistui {"response":1}
		} else {
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out = response.getWriter();
		Dao dao = new Dao();
		if(dao.muutaAsiakas(asiakas, vanhaid)) {	// metodi paluattaa true/false
			out.println("{\"response\":1}");	//Asiakkan lis‰‰minen onnistui {"response":1}
		} else {
			out.println("{\"response\":0}");	//Asiakkaan lis‰‰minen ep‰onnistui {"response":0}
		}
